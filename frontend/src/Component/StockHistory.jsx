  ...
  const groupDataByName = (data) => {
      ...
      data.forEach((item) => {
        ...
        if (existingEntry) {
          ...
          existingEntry.purchase += parseInt(item.purchase, 10);
          existingEntry.sales += parseInt(item.sales, 10);
          existingEntry.machine_use += parseInt(item.machine_use, 10);
          existingEntry.currentStock+= parseFloat(item.currentStock,10);
        } else {
        
          groupedData.push({
            itemName: item.itemName,
            itemCode:item.itemCode ,
            purchase: parseInt(item.purchase, 10),
            sales:parseFloat(item.sales, 10),
            machine_use:parseFloat(item.machine_use, 10),
            currentStock: parseInt(item.currentStock, 10),
            date:item.date,
            time:item.time
          });
        }
  ...
  return (
    <div sortActive={true}>
        ...
        <div className='heading_bill' >
          <span className='head-text'><img src={arrowIcon} alt="" />Stock History</span>
        <div style={{
            marginLeft:"50px"
        }}>
